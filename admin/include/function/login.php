...
{
	$r = mysql_query("select * from data_driver where password='$password' and username='$username'");
...
if (mysql_num_rows($r) == 1) 
{
		...
		setcookie('jenenge', $jenenge, time() + (3600 * 24), '/');


		$cek = cek_database("","","","select * from data_admin where username='$username' and password='$password'");
		if ($cek == "ada")
		{
			setcookie('siapa', "admin", time() + (3600 * 24), '/');
			setcookie('id', $data["id_admin"], time() + (3600 * 24), '/');
			setcookie('name', $data["username"], time() + (3600 * 24), '/');
		}
		else
		{
			setcookie('siapa', "driver", time() + (3600 * 24), '/');
			setcookie('id', $data["id_driver"], time() + (3600 * 24), '/');
			setcookie('name', $data["nama"], time() + (3600 * 24), '/');
		}
